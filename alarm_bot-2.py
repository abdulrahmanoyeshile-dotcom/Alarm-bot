...
# ── Keep-alive server (stops Replit free tier from sleeping) ──────────────────

keep_alive_app = Flask(__name__)

@keep_alive_app.route("/")
def home():
    return "Alarm Bot is running!"
...
def keep_alive():
    # Start Flask server
    server_thread = threading.Thread(target=lambda: keep_alive_app.run(host="0.0.0.0", port=8080))
